...
3. **Review** — Run code review on uncommitted changes, then fix reported issues
   ...
   - **Other tools**: run `coderabbit review --uncommitted` (CLI ≥ 0.7 dropped `--prompt-only`
     and `--plain` and made plain text the default; `--type` was replaced by the named scope flags
     but still parses as unlisted compatibility syntax — it is not an unknown option)
...
6. **Version & Changelog** — When changes warrant a release, bump version and update `CHANGELOG.md`. **The release commit MUST be the final commit of the release cycle** — if earlier finalize steps (simplify, review, docs, re-validate) revealed additional fixes, commit those **first** and only then run the release step. This guarantees `git checkout v<version>` matches the exact state you released and keeps changelog diffs (`git log vA..vB`) accurate.
   - Claude Code: invoke `/bump-version` (preferred — it is `/ak-git:operations --release` with this repository's version-file layout supplied: auto-detects the bump type from the commits since the last release commit, updates `marketplace.json`, every `plugins/*/.claude-plugin/plugin.json` and the `AGENTS.md` version line, runs `/ak-meta:changelog --no-commit`, makes one release commit, and creates an annotated tag)
   - Manual fallback: invoke `/ak-meta:changelog` directly (handles CHANGELOG only — version sync across the discovered files and tagging remain manual; see "Commit and PR guidelines" in AGENTS.md). Always create annotated tags (`git tag -a v<version> -m "Release v<version>"`), never lightweight tags.
