...
if [ "$EXIT_CODE" -ne 0 ] && [ "$EXIT_CODE" -ne 124 ] && [ "$EXIT_CODE" -ne 125 ] && [ -s "$RAW_OUTPUT_FILE" ]; then
  REFUSAL=$(jq -R 'fromjson? // empty' "$RAW_OUTPUT_FILE" 2> /dev/null \
    | jq -rs '[.[] | select(.type == "turn.failed")] | last | if . then (.error.message // "no reason given") else empty end' 2> /dev/null || echo "")
  if [ -n "$REFUSAL" ]; then
    EXIT_CODE=126
    echo "codex-adapter.sh: TOOL REFUSED - the run was ended by the tool itself, not by this adapter: ${REFUSAL}" >&2
    echo "codex-adapter.sh: this is exit 126, distinct from 124 (timed out, partial output worth salvaging) and 125 (never started, usually transient). Retrying immediately will hit the same wall." >&2
    echo "codex-adapter.sh: whatever the tool produced before refusing is still in $RAW_OUTPUT_FILE." >&2
  fi
fi
